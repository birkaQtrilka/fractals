...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    // Using the uniform to prevent it from being optimized away
    let dummy = uniforms.cell_size; 
    
    let smoke_val = textureSample(grid_texture, grid_sampler, in.uv).r;
